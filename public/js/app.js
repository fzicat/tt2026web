...
async function initializeApp() {
    ...
    if (!initSupabase()) {
        ...
        document.getElementById('login-view').innerHTML = `
            <div class="login-box">
                <h1>TradeTools</h1>
                <p class="error">Supabase is not configured. Please set SUPABASE_URL and SUPABASE_ANON_KEY.</p>
            </div>
        `;
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // If config is already loaded (from template), initialize immediately
    if (window.SUPABASE_URL && window.SUPABASE_ANON_KEY &&
        !window.SUPABASE_URL.includes('{{') && !window.SUPABASE_ANON_KEY.includes('{{')) {
        await initializeApp();
    }
    // Otherwise wait for configLoaded event (Vercel deployment)
});

// Listen for config loaded event (from /api/config)
document.addEventListener('configLoaded', async () => {
    await initializeApp();
});
